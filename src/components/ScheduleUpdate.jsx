...
function ScheduleUpdate({ onClose, scheduleData }) {
    ...
    const handleEditOpen = (index) => {
        const rowData = scheduleData[index];
        // Format the dates properly for the date input
        const formattedData = {
            ...rowData,
        ...
        setEditedRowIndex(index);
        setEditedLineData(formattedData);
    };
    const handleCloseEdit = () => {
        setEditedRowIndex(-1);
        setEditedLineData({});
                            ...
                            <tbody>
                                {scheduleData.map((row, index) => (
                                    <tr key={index}>
                                        <td>{editedRowIndex === index ? <input onChange={e => handleChange('place', e.target.value)} type="text" value={editedLineData.place || ''} /> : row.place}</td>
                                        <td>{editedRowIndex === index ? <input onChange={e => handleChange('projNo', e.target.value)} type="text" value={editedLineData.projNo || ''} /> : row.projNo}</td>
                                        <td>{editedRowIndex === index ? <input onChange={e => handleChange('startDate', e.target.value)} type="date" value={editedLineData.startDate || ''}  min="2000-01-01"
            max="2050-12-31"/> : row.startDate}</td>
                                        <td>{editedRowIndex === index ? <input onChange={e => handleChange('endDate', e.target.value)} type="date" value={editedLineData.endDate || ''}  min="2000-01-01"
            max="2050-12-31"/> : row.endDate}</td>
                                        {/* <td style={tableCellStyle}>{row.place}</td>
                                        <td style={tableCellStyle}>{row.projNo}</td>
                                        <td style={tableCellStyle}>{row.startDate}</td>
                                        <td style={tableCellStyle}>{row.endDate}</td> */}
